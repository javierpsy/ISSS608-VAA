...
Funnel plot is a specially designed data visualisation for conducting unbiased comparison between outlets.
In this hands-on Exercise, we'll gain hands-on experience on:
...
## 8.7.1 Additional Analysis 1 - Distribution of Fatality Rates by City

**Objective:** To compare how COVID-19 fatality rates vary across different cities in DKI Jakarta using boxplots.

```{r}
covid19_city_rate <- covid19 %>%
  mutate(fatality_rate = Death / Positive) %>%
  filter(Positive > 0)

ggplot(covid19_city_rate,
       aes(x = City, y = fatality_rate, fill = City)) +
  geom_boxplot(alpha = 0.7, outlier.color = "red") +
  labs(
    title = "Distribution of COVID-19 Fatality Rates by City",
    x = "City",
    y = "Fatality Rate"
  ) +
  theme_minimal() +
...
**Insights:** Some cities exhibit higher and more variable fatality rates across sub-districts, indicating differences in COVID-19 impact and healthcare burden.


## 8.7.2 Additional Analysis 2: Relationship Between Positive Cases and Deaths

**Objective:** To examine the relationship between cumulative positive COVID-19 cases and cumulative deaths across sub-districts.

```{r}
covid19 %>%
  filter(Positive > 0, Death >= 0) %>%
  ggplot(aes(x = Positive, y = Death)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(
    title = "Relationship Between Positive Cases and Deaths",
...
**Insights:** Sub-districts with higher positive cases generally tend to report higher deaths, suggesting a positive relationship between infection burden and mortality.

## 8.7.3 Additional Analysis 3: Top 10 Sub-districts by Fatality Rate

**Objective:** To identify the top 10 sub-districts with the highest COVID-19 fatality rates for targeted monitoring and public health intervention.

```{r}
covid19 %>%
  mutate(fatality_rate = Death / Positive) %>%
  filter(Positive >= 100, Death > 0) %>%
  arrange(desc(fatality_rate)) %>%
  slice_head(n = 10) %>%
  ggplot(aes(x = reorder(`Sub-district`, fatality_rate),
             y = fatality_rate)) +
  ...
  labs(
    title = "Top 10 Sub-districts by Fatality Rate",
    subtitle = "Sub-districts with at least 100 positive cases",
    x = "Sub-district",
    y = "Fatality Rate"
  ) +
...
**Insights:** Several sub-districts recorded significantly higher fatality rates relative to their positive cases, highlighting potential high-risk areas requiring closer investigation.
...
-   ggdist, a ggplot2 extension spacially design for visualising distribution and uncertainty,
...
[**ggstatsplot**](https://indrajeetpatil.github.io/ggstatsplot/index.html) ![](https://r4va.netlify.app/chap10/img/image1.jpg){width="49"} is an extension of [**ggplot2**](https://ggplot2.tidyverse.org/) package for creating graphics with details from statistical tests included in the information-rich plots themselves.

```         
- To provide alternative statistical inference methods by default. 
- To follow best practices for statistical reporting. For all statistical test:
```

![](/Hands-On_Ex/Hands-on_Ex04/Visual%20Statistical%20Analysis.png){width="683"}
...
```{r}
pacman::p_load(ggstatsplot, tidyverse, ggplot2)
```
...
![](/Hands-On_Ex/Hands-on_Ex04/Unpacking%20the%20Bayes%20Factor.png)
...
![](/Hands-On_Ex/Hands-on_Ex04/Bayes%20Factor.png){width="284"}
...
![](/Hands-On_Ex/Hands-on_Ex04/ggbetweenstats%20-%20Summary%20of%20tests.png){width="363"}
...
In this case, Exam_data.csv will be used.
...
Instead of plotting the standard error bar of point estimates, we can also plot the confidence intervals of mean maths score by
...
### 11.3.3 **Visualizing the uncertainty of point estimates with iteractive error bars**
...
![](/Hands-On_Ex/Hands-on_Ex04/ggdist%20package.png){width="270"}
...
## 11.6.1 Additional Analysis 4 - Class Consistency Analysis

**Objective:** To identify which classes demonstrate the most consistent Mathematics performance based on score variability.
...
ggplot(class_sd,
  ...
  labs(
    title = "Variation in Maths Scores by Class",
    x = "Class",
    y = "Standard Deviation of Maths Scores"
  ) +

  theme_minimal()
```

**Insights:** Classes with lower standard deviation indicate more consistent student performance, while higher variation suggests larger performance gaps among students.

## 11.6.2 Additional Analysis 5 - High Performer Composition by Race

**Objective:** To examine the distribution of high-performing students in Mathematics across different racial groups.
...
ggplot(high_perf,
  ...
  labs(
    title = "Number of High Performers in Maths by Race",
    x = "Race",
...
**Insights:** Some racial groups contribute a larger proportion of students scoring 85 and above in Mathematics, highlighting differences in top-performing student representation.

## 11.6.3 Additional Analysis 6 - Subject Balance Analysis

**Objective:** To analyse whether students perform consistently across Mathematics and English by measuring the score gap between the two subjects.

```{r}
exam %>%
  mutate(
    score_gap = abs(MATHS - ENGLISH)
  ) %>%
  ...
  labs(
    title = "Distribution of Subject Performance Gaps",
    subtitle = "Absolute difference between Maths and English scores",
    x = "Score Gap",
    y = "Number of Students"
  ) +
...
**Insights:** Most students exhibit relatively balanced performance between Mathematics and English, although some students show noticeably stronger performance in one subject over the other.
