...
fraud_nodes <- tibble(
  id = 1:25,
  label = paste0("Actor_", 1:25),
  Type = sample(
    c("Policyholder", "Workshop", "Claim Agent"),
    25,
    replace = TRUE
  )
)

# Create denser edge dataset
fraud_edges <- tibble(
  source = sample(1:25, 180, replace = TRUE),
  target = sample(1:25, 180, replace = TRUE)
) %>%
  filter(source != target) %>%
...
**Insight:** This analysis highlights actors with many claim-related connections. Larger nodes represent actors with higher degree centrality, meaning they interact with more policyholders, workshops, or claim agents. In a real insurance investigation, these highly connected actors may be useful starting points for detecting unusual claim behaviour or potential fraud networks.
