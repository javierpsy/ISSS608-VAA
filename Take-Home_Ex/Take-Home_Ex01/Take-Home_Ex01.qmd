...
## 4.4 Claim Cost Distribution across Policy Types

When exploring motor insurance data, one important target variable is the total incurred claim cost. This represents the total claim cost of each policy, including paid amounts and outstanding reserves.

However, insurance claim costs are usually highly right-skewed, where many policies have no claims while a small number of policies incur very large losses. Therefore, `log1p(total_incurred)` is used to improve readability while retaining zero-claim policies.
...
plot_policy_incurred <- function(data, selected_policy, chart_title) {
  
  plot_data <-
    data |>
    filter(
      policy_type == selected_policy,
      !is.na(log_total_incurred),
      is.finite(log_total_incurred),
      log_total_incurred >= 0
    )
  
  mean_value <- round(mean(plot_data$log_total_incurred, na.rm = TRUE), 2)
  median_value <- round(median(plot_data$log_total_incurred, na.rm = TRUE), 2)
  min_value <- round(min(plot_data$log_total_incurred, na.rm = TRUE), 2)
  max_value <- round(max(plot_data$log_total_incurred, na.rm = TRUE), 2)
  
  leftwhisk_value <- round(
    quantile(plot_data$log_total_incurred, probs = .25, na.rm = TRUE) -
      1.5 * IQR(plot_data$log_total_incurred, na.rm = TRUE),
    2
  )
  
  x_upper <- quantile(data$log_total_incurred, 0.99, na.rm = TRUE)
  
  hist_plot <-
    ggplot(plot_data, aes(x = log_total_incurred)) +
    geom_histogram(
      bins = 25,
      color = "#f5f5f5",
      fill = "#c7c8cc"
  ...
  box_plot <-
    ggplot(plot_data, aes(y = log_total_incurred, x = "")) +
    geom_boxplot(
      fill = "#c7c8cc",
      outlier.colour = "#d18a7d",
      outlier.shape = 16,
...
```{r}
plot_policy_incurred(
  motor_feature,
  "Third-party",
  "Distribution of Log Total Incurred for Third-party Policies"
)
```
...
plot_policy_incurred(
  motor_feature,
  "Third-party + Glass",
  "Distribution of Log Total Incurred for Third-party + Glass Policies"
)
```
...
plot_policy_incurred(
  motor_feature,
  "Combined Coverage",
  "Distribution of Log Total Incurred for Combined Coverage Policies"
)
```
...
plot_policy_incurred(
  motor_feature,
  "Comprehensive with Excess",
  "Distribution of Log Total Incurred for Comprehensive with Excess Policies"
)
```
...
plot_policy_incurred(
  motor_feature,
  "Comprehensive without Excess",
  "Distribution of Log Total Incurred for Comprehensive without Excess Policies"
)
```
...
- The distribution of incurred claim cost remains right-skewed across policy types, even after logarithmic transformation.

- Most policies are concentrated at the lower end of incurred cost, indicating that many policyholders recorded no or low claim amounts.

- The mean values are generally higher than the median values, suggesting that a smaller group of higher-cost claims pulls the average upward.

- Comprehensive policy products appear to show wider spread and more extreme observations, indicating greater claim cost variability.

- This suggests that policy type is an important factor to consider when analysing claim cost risk and portfolio profitability.
