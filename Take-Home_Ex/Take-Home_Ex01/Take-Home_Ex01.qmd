...
underpricing_plot <-
  risk_summary |>
  mutate(
    policy_type = reorder(policy_type, underpricing_rate),
    exposure_label = paste0("Policies: ", scales::comma(total_policies))
  ) |>
  ggplot(
    aes(
      x = underpricing_rate,
  ...
  geom_text(
    aes(
      label = scales::percent(underpricing_rate, accuracy = 0.1)
    ),
    hjust = -0.15,
    colour = "#E0218A",
    size = 3.6,
  ...
  annotate(
    "text",
    x = max(risk_summary$underpricing_rate, na.rm = TRUE) * 0.70,
  ...
  labs(
    title = "Underpricing Risk across Policy Types",
    subtitle = "Underpricing rate measures the share of policies where incurred claims exceed collected premiums",
    x = "Underpricing Rate",
    y = NULL
  ) +
  theme_minimal() +
...
-   The underpricing analysis indicates that Comprehensive without Excess products face significantly greater pricing adequacy pressure, with 20.8% of policies experiencing incurred claims exceeding collected premiums, highlighting elevated underwriting and profitability risk.
...
## 5.4 Are Certain Policy Products More Vulnerable to Underpricing Risk?
