...
claim_status_plot <- motor_feature |>
  count(claim_status) |>
  mutate(
    pct = n / sum(n),
  ...
  labs(
    title = "Most Policies Did Not Report Claims",
    subtitle = "Claim status composition across the motor insurance portfolio",
    x = NULL,
...
-   Premium values exhibit a positively skewed distribution, where most policies are concentrated within lower-to-moderate premium ranges while a smaller number of policies possess substantially higher premiums.

-   The concentration around the median premium level suggests that the portfolio mainly consists of standard-priced policies, with fewer high-premium policies contributing to the long tail.

-   The extended right tail indicates heterogeneity within the insurance portfolio and may reflect policies with higher vehicle values, broader coverage types, or policyholders with elevated risk characteristics.

-   Such variability suggests that portfolio pricing is not evenly distributed and motivates further investigation into customer and vehicle attributes associated with premium differences.
...
-   The ages of drivers are mainly focused between around 35 and 60 years, indicating that the portfolio consists predominantly of middle-aged policyholders.

-   An average age of approximately 48 years suggests that the usual insured client is part of a fairly older demographic group.

-   Drivers who are younger or older represent a reduced share of the portfolio and might display varying characteristics of insurance risk.

-   The age distribution indicates that the existing portfolio might be dominated by seasoned drivers instead of recently licensed policyholders.
:::
...
driving_exp_plot <- ggplot(
  ...
  labs(
    ...
    subtitle = "Distribution of estimated driving experience by experience group",
    x = "Driving Experience Group",
    y = "Driving Experience"
  ) +
  theme_minimal() +
  theme(
...
-   Highly experienced drivers show the highest median driving exposure and the widest spread, suggesting substantial variability within this customer segment.

-   Several outliers are observed among highly experienced drivers, indicating a small group with exceptionally long driving histories beyond the typical range.

-   New drivers display a narrow and concentrated distribution with consistently low driving experience, reflecting a relatively homogeneous profile.

-   The clear separation across experience categories suggests that the portfolio consists of multiple driving maturity segments, which may exhibit different insurance risk characteristics and behavioural patterns.
...
vehicle_value_segment_plot <-
  ...
  labs(
    title = "Vehicle Value Segment Composition",
    subtitle = "Distribution of insured vehicles across fixed value bands",
