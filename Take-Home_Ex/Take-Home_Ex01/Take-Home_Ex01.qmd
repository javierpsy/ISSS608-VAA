...
Before conducting downstream analysis, selected categorical variables were reviewed and converted into appropriate factor formats. Several categorical variables in the dataset are stored as character values. These variables were converted into factor formats to support grouping, segmentation, and clearer visualisation in subsequent analyses.

The conversion was applied to selected categorical variables as shown below:

```{r}
motor_convert <- motor_select %>%
mutate(
...
The conversion process transformed selected categorical variables into factor representations to improve interpretability during visual analysis. The `bonus_score` variable was represented as an ordered factor because it reflects a ranking structure associated with claims and driving history.
