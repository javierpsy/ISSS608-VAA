...
-   The red vertical line represents the median premium level and serves as a reference for the typical pricing point within the insurance portfolio.
...
## 4.4 Hidden Risk Indicators

Finally, engineered indicators were examined to uncover portfolio patterns that are not directly observable from the original variables. Unlike raw claim counts, these derived indicators provide a more meaningful view of claim behaviour, claim size, and portfolio loss performance.

Similar to the statistical distribution approach used earlier, each tab combines a boxplot and histogram. The boxplot highlights the spread, central tendency, and possible outliers, while the histogram shows the frequency distribution and skewness of each engineered indicator.

::: panel-tabset

### Claim Frequency

Claim frequency measures the number of claims relative to policy exposure. This helps identify whether claims are broadly distributed across the portfolio or concentrated among a smaller subset of policies.

```{r}
claim_frequency_data <-
...
mean_claim_frequency <-
  ...
  round(
    mean(
      claim_frequency_data$claim_frequency,
      na.rm = TRUE
    ),
    2
  )
...
hist_claim_frequency <-
  ...
  geom_vline(
    xintercept = median_claim_frequency,
    colour = "#595DE5",
    linewidth = 0.7,
    linetype = "dashed"
  ) +
  ...
  geom_vline(
    xintercept = mean_claim_frequency,
    colour = "#e0218a",
    linewidth = 0.7,
    linetype = "dashed"
  ) +
...
box_claim_frequency / hist_claim_frequency +
  plot_layout(
    heights = c(
      1,
      3.5
...
Claim severity measures the average incurred amount per claim. This indicator is useful because two policies may have the same claim frequency but very different financial impact depending on the size of the claims.
...
mean_claim_severity <-
  ...
  round(
    mean(
      claim_severity_data$claim_severity,
      na.rm = TRUE
    ),
    0
  )
...
hist_claim_severity <-
  ...
  annotate(
    "text",
    x = median_claim_severity,
    y = Inf,
    label = paste0(
      "Median = ",
      scales::comma(median_claim_severity)
    ),
    colour = "#595DE5",
    vjust = 2,
    hjust = -0.05,
  ...
  annotate(
    "text",
    x = mean_claim_severity,
    y = Inf,
    label = paste0(
      "Mean = ",
      scales::comma(mean_claim_severity)
    ),
    colour = "#e0218a",
    vjust = 4,
    hjust = -0.05,
...
box_claim_severity / hist_claim_severity +
  plot_layout(
    heights = c(
      1,
      3.5
...
Loss ratio compares incurred losses against premium collected. This is an important insurance performance indicator because higher loss ratios may indicate policies where claims costs exceed or approach premium income.
...
box_loss_ratio / hist_loss_ratio +
  plot_layout(
    heights = c(
      1,
      3.5
...
- Claim frequency, claim severity, and loss ratio are strongly right-skewed, indicating that most policies generate low claim activity or limited financial loss while a smaller subset contributes higher risk outcomes.

- The comparison between mean and median values helps reveal skewness. When the mean is greater than the median, it suggests that extreme observations are pulling the average upward.

- Claim severity provides insight into the financial magnitude of claims, while claim frequency focuses on how often claims occur. Both indicators are needed to understand portfolio risk.

- Loss ratio links claims back to premium income and provides a more direct view of whether policy losses are proportionate to the premiums collected.

- These engineered risk indicators provide a strong foundation for subsequent bivariate analysis, where claim and loss behaviour can be compared across customer, vehicle, and policy characteristics.
