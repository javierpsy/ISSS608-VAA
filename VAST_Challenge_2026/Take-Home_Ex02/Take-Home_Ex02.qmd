...
pacman::p_load(
  ...
  ggiraph, readr, ggalluvial, visNetwork, gt, visNetwork, tidytext
)
```
...
section4_evidence_summary <- tibble(
  ...
  `Key Evidence from Section 4` = c(
    "Daily communication activity remained relatively stable at around 25–40 messages daily until they surged to over 500 messages on June 5, 2046.",
    
    "Moderate-risk and high-risk communication signals  increased significantly during the critical breach period, indicating that communication behavior grew more unusual close to the inappropriate release.",
    
    "Internal group communication remained dominant, but its share dropped from approximately 61% during the normal period to 46% during the critical period, while side-channel communication increased from about 4% to 19%.",
    
    "Controlled and low-exposure communication activity became more prominent during the critical period, while moderate and high exposure behaviours were already observable before the breach window.",
    ...
    "The critical-period network revealed a tightly connected high-risk communication cluster involving Legal, PR, Social Media, and Platform Trust, with thicker directional flows indicating repeated coordination behaviour.",
    
    "Earlier warning indicators existed before June 3, 2046, but appeared at lower intensity. Moderate-to-high risk communications increased from 49 before the critical period to 337 during the critical period."
  ),
...
-   The exploratory analysis indicates that the inappropriate release was not preceded by a single isolated signal. Instead, several warning patterns gradually emerged across communication volume, behavioural risk signals, communication environments, high-risk pathways, and earlier warning indicators.

-   Although, the earlier warning signs appeared weaker and less concentrated compared to the escalation observed during the critical breach period. This may explain why prior occasions did not result in stronger intervention or containment action.
...
Rather than showing the full communication network, the analysis focuses on the direct ego network of the most active participant. This helps reveal the immediate communication relationships surrounding the breach-related evidence trail.
...
-   Elevated-risk communication was concentrated among a small group of participants rather than being distributed evenly across the communication network, suggesting that the circulation of embargo-sensitive information was driven by a focused operational cluster.

-   Legal and PR functioned as coordinators, exhibiting both high incoming and outgoing communication activity. Their positions within the network indicate that they acted as intermediaries connecting multiple communication pathways.

-   Social Media displayed broadcaster characteristics, contributing a larger proportion of outgoing communication relative to incoming communication. This role is relevant given the eventual public release of embargoed information.

-   Platform Trust and Social Manager appeared more receiver-oriented, indicating that they were more frequently involved in receiving elevated-risk communications than initiating them, suggesting downstream involvement in the communication flow.

-   The concentration of communication activity around Legal, PR, Social Media, Platform Trust, and Social Manager highlights a potential pathway through which embargo-sensitive information could have propagated prior to the inappropriate release.
:::

## 5.3 What Communication Patterns Emerged Prior to the Inappropriate Release?

This section examines if communication behavior shifted before the improper disclosure of restricted information. Instead of concentrating on specific actors, the analysis looks at communication content to detect emerging patterns that could have acted as early warning indicators prior to the breach happening.

Using text mining techniques, communication logs are analyzed across the Normal Period, Escalation Period, and Critical Period. The aim is to assess if the communication content became more concentrated on particular subjects or displayed atypical traits as the release date neared.
...
- Communication topics significantly changed throughout the three communicatio periods. While the Normal and Escalation Periods were characterised by operational and procedural discussions, the Critical Period introduced several terms associated with governance, platform trust, social media management, and the HarborCrest–CivicLoom context.
...
- The emergence of governance- and release-related topics pbefore the breach indicates possible early signs that the communication context was turning more sensitive and more linked to the restricted information.

- Overall, the TF-IDF analysis suggests that the inappropriate release was preceded by a measurable shift in communication content rather than occurring as a completely isolated event.
:::
...
-   Communications became increasingly centred around governance, auditing, access control, and platform trust, indicating a shift toward oversight and accountability-related discussions.
...
-   In general, these recurring communication themes provide evidence that the inappropriate release was preceded by identifiable governance- and communication-related discussions, rather than occurring in isolation.
:::

## 5.5 What Communication Pathway Likely Led to the Inappropriate Release?
...
### 5.5.1 Critical Communication Timeline
...
ggplot(
  ...
  labs(
    title = "Elevated-risk Communications Increased Near the Release",
    subtitle = "Hourly count of moderate- and high-risk communications during the Critical Period",
    x = NULL,
    y = "Number of communications",
    fill = "Risk Signal"
  ) +
...
### 5.5.2 Key Events Leading to the Release
...
### 5.5.3 Focused Communication Pathway Network
...
pathway_edges_55 <- communication_edges %>%
  mutate(
    communication_date = as.Date(timestamp),
    source = clean_actor(source),
    target = clean_actor(target)
  ...
  filter(
    communication_date >= as.Date("2046-06-04"),
    communication_date <= as.Date("2046-06-05"),
    risk_signal_group %in% c("Moderate Signal", "High Signal"),
    !is.na(source),
    !is.na(target),
    source != "",
...
pathway_nodes_55 <- tibble(
  id = unique(c(pathway_edges_55$source, pathway_edges_55$target))
) %>%
  mutate(
    label = id,
    value = 1,
...
pathway_edges_vis_55 <- pathway_edges_55 %>%
  transmute(
    from = source,
    to = target,
    value = weight,
    arrows = "to",
    width = scales::rescale(weight, to = c(1, 7)),
    color = if_else(
      dominant_risk_signal == "High Signal",
      "#cb181d",
      "#fdae6b"
    ...
    title = paste0(
      "<b>", source, " → ", target, "</b><br>",
      "Total interactions: ", weight, "<br>",
      "Dominant signal: ", dominant_risk_signal, "<br>",
      "High-signal interactions: ", high_signal_weight, "<br>",
      "Moderate-signal interactions: ", moderate_signal_weight
    )
  )
...
visNetwork(
  pathway_nodes_55,
  pathway_edges_vis_55,
  height = "500px",
  width = "100%",
  main = "Likely Communication Pathway During the Critical Period"
) %>%
  visNodes(
    shape = "dot",
    color = list(background = "#9ecae1", border = "#2171b5"),
...
- Elevated-risk communications became concentrated near the release window, indicating that the breach was preceded by an observable escalation in communication activity.

- The chronological event table helps identify the order in which elevated-risk communications appeared, supporting reconstruction of the likely sequence leading to the release.

- The focused pathway network shows which actors were connected through repeated moderate- or high-risk communications during the most critical period.

- Red pathways represent high-risk dominant communication links, highlighting where information movement may have been most sensitive.

- Taken together, the timeline and network provide evidence that the inappropriate release was linked to a sequence of elevated-risk communications rather than a single isolated message.
