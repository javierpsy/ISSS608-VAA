...
pacman::p_load(
  ...
  ggiraph, readr, ggalluvial, visNetwork, gt, visNetwork, tidytext, ggrepel
)
```
...
-   Elevated-risk communication was concentrated among a small group of participants rather than being distributed evenly across the communication network, suggesting that embargo-sensitive information circulated within a focused operational cluster.
...
-   The recurring phrases suggest that discussions during the Critical Period were increasingly centred on governance, access controls, audits, and public communication activities.
...
### 5.6.1 Behavioural Shift by Actor Role
...
ggplot(
  role_deviation_56,
  aes(y = reorder(agent_role, change))
) +
  geom_segment(
    aes(
      x = `Earlier Periods`,
      xend = `Critical Period`,
      yend = agent_role
    ),
    linewidth = 1.2,
    colour = "grey70"
  ...
  geom_point(
    aes(x = `Earlier Periods`, colour = "Earlier Periods"),
    size = 4
  ) +
  geom_point(
    aes(x = `Critical Period`, colour = "Critical Period"),
    size = 4
  ) +
  geom_text(
    aes(
      x = pmax(`Earlier Periods`, `Critical Period`) + 8,
      label = change_label
    ),
    size = 3.5,
    fontface = "bold"
  ...
  labs(
    title = "Behavioural Deviation by Actor Role",
    subtitle = "Comparing earlier communication activity with Critical Period activity",
...
### 5.6.2 Expected vs Observed Behaviour
...
### 5.6.3 Elevated-risk Communications Across Breach Windows
...
-   Legal (+98 communications) and Social Media (+78 communications) exhibited the largest departures from their typical communication behaviour during the Critical Period. Legal became a key coordinator, while Social Media assumed a more prominent broadcasting role. 

-   The Expected vs Observed Behaviour comparison shows that communication responsibilities shifted during the breach period, with some participants becoming substantially more active while others became less prominent than in earlier periods.

-   Elevated-risk communications were present during the Normal and Escalation Periods, indicating that warning signals existed before the breach. However, the Critical Period recorded substantially higher levels of Moderate Signal and High Signal communications than earlier periods.
...
This study explored the communication practices, behavioral trends, and network dynamics that occurred before the inappropriate release of embargo-sensitive information.
...
Network analysis showed that Legal, PR, Social Media, and Platform Trust formed the main communication cluster linked to elevated-risk interactions. During the Critical Period, Legal and Social Media also became noticeably more active than in earlier periods, suggesting that participant behaviour changed as the release approached.

Text analysis also revealed that communication subjects shifted to emphasize governance, trust in the platform, access controls, auditing, and public relations. This suggests that the communication context grew more intertwined with matters related to the eventual revelation.
...
In summary, the improper release seems to have stemmed from a mix of behavioural escalation, focused communication channels, high-risk interactions, and changing roles of participants. These findings are broadly aligned with prior research demonstrating that communication networks often exhibit changes in structure, participant behaviour, and information flow patterns before major organisational events [(Wen et al., 2024)](https://www.sciencedirect.com/science/article/pii/S0306457324002863?via%3Dihub)
...
summary_evidence_tbl <- tibble(
  ...
  `Key Finding` = c(
    ...
    "Yes. Earlier warning signals existed before the breach, but they occurred at lower intensity and involved fewer participants.",
...
Chapter 27: Modelling, Visualising and Analysing Network Data with R – R for Visual Analytics. (n.d.). https://r4va.netlify.app/chap27

Chapter 29: Visualising and Analysing Text Data with R: tidytext methods – R for Visual Analytics. (n.d.). https://r4va.netlify.app/chap29

Wen, T., Chen, Y., Syed, T. A., & Ghataoura, D. (2024). Examining communication network behaviors, structure and dynamics in an organizational hierarchy: A social network analysis approach. Information Processing & Management, 62(1), 103927. https://doi.org/10.1016/j.ipm.2024.103927